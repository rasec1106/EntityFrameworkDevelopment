...
namespace ApiProduct.Repository
{
    public class ProductSQLRepository : IProductRepository
    {
        ...
        public async Task<IEnumerable<ProductDto>> GetProductsByCategoryCode(string categoryCode)
        {
            List<Product> products = await dbContext.Products.Where(product =>
            product.CategoryCode == categoryCode).ToListAsync();
